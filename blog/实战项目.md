...
一个简单的时钟，可以实现在几分钟或几秒钟，或者在一个特定的事件播放音乐

**Distance Between Two Cities** - Calculates the distance between two cities and allows the user to specify a unit of distance. This program may require finding coordinates for the cities like latitude and longitude.

计算两个城市之间的距离，并允许用户制定一个单位。这个程序可能会要求去找到城市的经纬度
...
一个快乐的数字表示，用这个数字的每个位数加起来替换掉这个数，如果这个数字最后停在1或者形成了一个圈
...
编写一个程序，模拟掷硬币的过程，并记录下来正反面的次数

**Limit Calculator** - Ask the user to enter f(x) and the limit value, then return the value of the limit statement *Optional: Make the calculator capable of supporting infinite limits.*

允许程序支持输入f(x)和限制数字，然后返回这个程序的值

**Fast Exponentiation** - Ask the user to enter 2 integers a and b and output a^b (i.e. pow(a,b)) in O(lg n) time complexity.

快速模幂运算，要求用户输入两个值a和b，然后计算出pow(a,b)的O(lg n)的值
...
在度量的几何模型内，找到一对点使得这对点距离给定的n对点距离总和最短

**Sieve of Eratosthenes** - The sieve of Eratosthenes is one of the most efficient ways to find all of the smaller primes (below 10 million or so).

实现Eratosthenes 找质素方法
